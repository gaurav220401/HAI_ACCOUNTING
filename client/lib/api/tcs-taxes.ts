...
export const TCS_SECTIONS = [
  ...
  { code: "194IA", label: "194IA - TCS on immovable property sale" },
...
export interface TcsTax {
  ...
  tcsPayableAccountId?: { _id: string; name: string; accountType: string } | string | null;
  tcsReceivableAccountId?: { _id: string; name: string; accountType: string } | string | null;
...
export interface CreateTcsTaxInput {
  taxName: string;
  rate: number;
  sectionCode: string;
  sectionDescription?: string;
  tcsPayableAccountId?: string | null;
  tcsReceivableAccountId?: string | null;
  isHigherRate?: boolean;
  applicableStartDate?: string | null;
  applicableEndDate?: string | null;
}

export type UpdateTcsTaxInput = Partial<CreateTcsTaxInput> & { isActive?: boolean };

export const tcsTaxApi = {
  ...
  update: (id: string, data: UpdateTcsTaxInput) =>
    apiFetch<{ data: TcsTax }>(`/tcs-taxes/${id}`, { method: "PATCH", body: JSON.stringify(data), headers: { "Content-Type": "application/json" } }),
  remove: (id: string) => apiFetch<{ success: boolean }>(`/tcs-taxes/${id}`, { method: "DELETE" }),
};
