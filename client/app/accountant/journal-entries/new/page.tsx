...
function NewJournalPageInner() {
  ...
  const [journalNumber, setJournalNumber] = useState("journal1");
  ...
  const updateRow = (id: string, field: keyof JournalRow, value: JournalRow[keyof JournalRow]) =>
    setRows((prev) =>
  ...
  const handleSubmit = async (publish: boolean) => {
    ...
    try {
      ...
      // Build a Journal object matching the list page's type
      ...
      // Persist to localStorage (same key the list page uses)
      const STORAGE_KEY = "hai_journals";
      const existing = JSON.parse(localStorage.getItem(STORAGE_KEY) || "[]");
      localStorage.setItem(
        STORAGE_KEY,
        JSON.stringify([newJournal, ...existing])
      );
