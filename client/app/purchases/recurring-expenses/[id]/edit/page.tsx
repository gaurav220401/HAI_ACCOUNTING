...
interface FormData {
  profileName: string; freqKey: string; startDate: string; neverExpires: boolean;
  endsOn: string; expenseAccountId: string; currency: string; amount: string;
  paidThroughAccountId: string; vendorId: string; notes: string;
  customerId: string; isBillable: boolean; projectId: string;
}
const DEFAULT: FormData = {
  profileName: "", freqKey: "Weekly_1", startDate: new Date().toISOString().slice(0, 10),
  neverExpires: true, endsOn: "", expenseAccountId: "", currency: "INR", amount: "",
  paidThroughAccountId: "", vendorId: "", notes: "", customerId: "", isBillable: false, projectId: "",
};

function AccountSelect({ accounts, value, onChange, placeholder = "Select an account" }: {
      ...
      <SelectContent className="max-h-64">
        {grouped.map(([type, accs]) => (
          <SelectGroup key={type}>
            <SelectLabel className="text-[10px] uppercase tracking-wide text-muted-foreground font-semibold px-2">{type}</SelectLabel>
            {accs.map((a) => <SelectItem key={a._id} value={a._id} className="text-sm">{a.name}</SelectItem>)}
          </SelectGroup>
        ))}
      </SelectContent>
    </Select>
...
function ContactCombobox({ contacts, value, onChange, placeholder = "Select" }: {
  contacts: Contact[]; value: string; onChange: (v: string) => void; placeholder?: string;
}) {
  ...
  return (
    <div className="relative">
      <button type="button" className="w-full h-9 border rounded-md text-sm px-3 flex items-center justify-between bg-background hover:bg-muted/30" onClick={() => setOpen((o) => !o)}>
        <span className={selected ? "text-foreground text-sm" : "text-muted-foreground text-sm"}>{selected ? (selected.displayName || selected.companyName) : placeholder}</span>
      ...
      {open && (
        <div className="absolute z-50 w-full mt-1 bg-popover border rounded-md shadow-md">
          <div className="p-2 border-b flex items-center gap-1.5">
            ...
            <input autoFocus className="text-xs outline-none bg-transparent w-full" placeholder="Search…" value={search} onChange={(e) => setSearch(e.target.value)} />
          </div>
          <div className="max-h-48 overflow-y-auto">
            <button type="button" className="w-full text-left px-3 py-2 text-sm text-muted-foreground hover:bg-muted/40" onClick={() => { onChange(""); setOpen(false); setSearch(""); }}>— None —</button>
            {filtered.map((c) => (
              <button key={c._id} type="button" className="w-full text-left px-3 py-2 text-sm hover:bg-muted/40" onClick={() => { onChange(c._id); setOpen(false); setSearch(""); }}>
...
export default function EditRecurringExpensePage() {
  ...
  useEffect(() => {
    if (!params?.id || authLoading || !firebaseUser) return;
    recurringExpenseApi.getById(params.id).then((res) => {
      const rec = res.data;
      ...
      setForm({
        ...
        projectId: extractId(rec.projectId),
      });
    }).catch(() => toast.error("Failed to load")).finally(() => setLoading(false));
  }, [params?.id, authLoading, firebaseUser]);

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!form.profileName.trim() || !form.startDate || !form.amount) {
    ...
    setSubmitting(true);
    try {
      await recurringExpenseApi.update(params!.id, payload);
  ...
  if (authLoading || loading) {
    return <div className="flex min-h-svh items-center justify-center"><Loader2 className="h-6 w-6 animate-spin text-muted-foreground" /></div>;
  }
