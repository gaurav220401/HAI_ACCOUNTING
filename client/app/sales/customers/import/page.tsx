...
const FIELD_GROUPS = [
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "accountsReceivableAccount", label: "Accounts Receivable Account" },
...
export default function CustomerImportPage() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("hai_customer_import_mapping");
      if (saved) {
  ...
  const handleDownloadSample = async (format: "csv" | "excel") => {
    try {
      const blob = await contactApi.downloadSampleTemplate(format, "customer");
      const url = URL.createObjectURL(blob);
      const link = document.createElement("a");
      const filename = format === "excel" ? "sample_customers.xlsx" : "sample_customers.csv";
  ...
  const handleDownloadBlank = async (format: "csv" | "excel") => {
    try {
      const blob = await contactApi.downloadBlankTemplate(format, "customer");
      const url = URL.createObjectURL(blob);
      const link = document.createElement("a");
      const filename = format === "excel" ? "blank_customers.xlsx" : "blank_customers.csv";
      link.setAttribute("href", url);
      link.setAttribute("download", filename);
      link.style.visibility = "hidden";
      document.body.appendChild(link);
  ...
  const handleNextFromStep1 = async () => {
    ...
    try {
      ...
      headers.forEach(h => {
        ...
        else if (normalized === "creditlimit") mapping.creditLimit = h;
        else if (normalized === "salesperson") mapping.salesPerson = h;
        ...
        else if (normalized === "accountsreceivableaccount") mapping.accountsReceivableAccount = h;
      });
  ...
  const handleNextFromStep2 = async () => {
    ...
    try {
      ...
      formData.append("defaultContactType", "Customer");
  ...
  const handleConfirmImport = async () => {
    ...
    try {
      ...
      if (importRes?.data) {
        if (saveMapping) {
          try {
            localStorage.setItem("hai_customer_import_mapping", JSON.stringify(fieldMapping));
          } catch (err) {
      ...
      <main className="flex-1 p-8 max-w-5xl mx-auto w-full">
                ...
                >
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="space-y-0.5">
                      ...
                      <p className="text-xs text-slate-500">
                        Imports the duplicates in the import file, overwriting the details of the existing customers.
                      </p>
                    </div>
                  </div>
                </RadioGroup>
        ...
        {step === 3 && (() => {
          ...
          const downloadSkippedRowsCSV = () => {
            ...
            const link = document.createElement("a");
            link.setAttribute("href", url);
            link.setAttribute("download", "skipped_customers.csv");
            link.style.visibility = "hidden";
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);
            ...
            <div className="space-y-6">
              {readyItems.length === 0 ? (
                <div className="bg-red-50 border border-red-200 rounded-lg p-4 flex items-start gap-3 text-red-800 text-sm">
                  ...
                  <span className="font-semibold">None of the customers can be imported</span>
                </div>
              ) : (
                <div className="bg-green-50 border border-green-200 rounded-lg p-4 flex items-start gap-3 text-green-800 text-sm">
                  ...
                  <span className="font-semibold">
                    {readyItems.length} of the {mappedItems.length} customers are ready to be imported.
                  </span>
                </div>
              )}
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 text-sm font-semibold text-slate-800">
                      <span>Customers that are ready to be imported</span>
                ...
                <div className="space-y-3 pt-4">
                  ...
                  {showUnmappedDetails && unmappedHeaders.length > 0 && (
                    <div className="space-y-3 pl-2 mt-2">
                      <p className="text-xs text-slate-500">
                        The following fields in your import file have not been mapped to any Customer field. The data in these fields will be ignored.
                      </p>
