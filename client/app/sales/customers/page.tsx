      ...
      <Dialog open={exportModalOpen} onOpenChange={setExportModalOpen}>
        <DialogContent className="max-w-md p-0 gap-0 bg-white" showCloseButton={false}>
          <DialogHeader className="px-6 pt-5 pb-4 border-b flex flex-row items-center justify-between border-slate-100">
            <DialogTitle className="text-base font-semibold text-slate-800">Export Customers</DialogTitle>
            <button
          ...
          <div className="px-6 py-5 space-y-5 max-h-[70vh] overflow-y-auto">
            ...
            <div className="space-y-1.5">
              ...
              <select
                ...
                className="h-9 w-full rounded-md border border-slate-300 bg-white px-3 text-sm focus:outline-none text-slate-800 cursor-pointer"
              >
                <option value="Customers">Customers</option>
                <option value="Customer's Contact Persons">Customer's Contact Persons</option>
                <option value="Customer's Addresses">Customer's Addresses</option>
              </select>
            </div>
            ...
            <div className="space-y-3">
              ...
              >
                <div className="flex items-center space-x-2">
                  ...
                  <Label htmlFor="r-all" className="text-sm font-normal text-slate-700 cursor-pointer">All Customers</Label>
                </div>
                <div className="flex items-center space-x-2">
                ...
                >
                  <span className="truncate">
                    {(() => {
                      ...
                      const allTemplates = [...defaults, ...exportTemplates];
                      const selectedTemplate = allTemplates.find(t => t.id === selectedTemplateId) || defaults[0];
                      return selectedTemplate ? selectedTemplate.name : "Select an Export Template";
                    })()}
                  </span>
                  ...
                  <div className="absolute z-50 w-full overflow-hidden rounded-md border border-slate-200 bg-white shadow-lg mt-1">
                    ...
                    <div className="max-h-48 overflow-y-auto p-1 bg-white">
                      {(() => {
                        const defaults = defaultTemplatesByModule[exportModule] || [];
                        const allTemplates = [...defaults, ...exportTemplates];
                        const filtered = allTemplates.filter(t => t.name.toLowerCase().includes(templateSearchQuery.toLowerCase()));
                    ...
                    <div className="border-t border-slate-100 p-1 bg-slate-50">
                      <button
                        ...
                        onClick={() => {
                          ...
                          setNewTemplateName("");
                          const fields = getAvailableFieldsForModule();
                          setNewTemplateFields([{ backendField: fields[0]?.value || "displayName", exportHeader: fields[0]?.label || "Display Name" }]);
            ...
            <p className="text-xs text-slate-500 border-t border-slate-100 pt-3 leading-normal">
              <strong>Note:</strong> You can export only the first 25,000 rows. If you have more rows, please initiate a backup for the data in your organization, and download it. <span className="text-primary hover:underline cursor-pointer font-medium">Backup Your Data</span>
            </p>
          </div>
          
          <div className="px-6 py-4 border-t border-slate-100 bg-slate-50 flex items-center justify-start gap-2">
            <Button size="sm" onClick={handleExportCustomersSubmit} className="bg-primary hover:bg-primary/95 text-white font-medium px-4 h-8 text-xs cursor-pointer">
              Export
            </Button>
