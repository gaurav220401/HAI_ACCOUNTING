...
function getCustomerName(v?: Quote["customerId"]) {
  if (!v) return "";
  if (typeof v === "object") {
...
function getCustomerEmail(v?: Quote["customerId"]) {
  if (!v || typeof v === "string") return "";
  return (v as any).email || "";
}
...
function buildDefaultBody(params: {
  ...
  return `
    ...
    <p>Please find the quotation (<strong>${escapeHtml(params.quoteNumber)}</strong>) attached with this email.</p>
    ...
    <table style="border-collapse:collapse;font-size:18px;line-height:1.8;">
      <tr>
        <td style="padding-right:18px;"><strong>Quote Date</strong></td>
        <td style="padding-right:18px;">:</td>
        <td><strong>${escapeHtml(params.quoteDate)}</strong></td>
      </tr>
      <tr>
...
export default function SendQuoteEmailPage() {
  ...
  useEffect(() => {
    let mounted = true;
    async function load() {
      if (!quoteId || !firebaseUser || !activeOrganization?._id) return;
      setFetching(true);
      try {
        const res = await quoteApi.getById(quoteId);
        if (!mounted) return;
        const nextQuote = res.data;
        setQuote(nextQuote);

        const customerName = getCustomerName(nextQuote.customerId);
        ...
        setBody(
          buildDefaultBody({
            ...
            quoteDate: formatDate(nextQuote.quoteDate),
            amountText: `INR ${formatAmount(Number(nextQuote.total || 0))}`,
            senderName,
  ...
  useEffect(() => {
    async function loadPdfPreview() {
      ...
      try {
        const pdfBlob = await quoteApi.downloadPdf(quote._id);
  ...
  const title = useMemo(() => {
    if (!quote) return "Send Quote Email";
    return `Send Email - ${quote.quoteNumber}`;
  }, [quote]);
        ...
        <PageHeader
          breadcrumb={
            <div className="flex items-center gap-1.5 text-xs">
              <span className="font-semibold text-teal-700">Quotes</span>
            ...
            <div className="max-w-4xl mx-auto space-y-6">
              <div className="border border-slate-200 rounded-xl bg-white overflow-hidden shadow-2xs">
                <div className="px-6 py-4 border-b border-slate-200 text-lg font-bold text-slate-800">
                  Email To {getCustomerName(quote?.customerId) || "Customer"}
                </div>
                ...
                <div className="divide-y divide-slate-100">
                  ...
                  <div className="px-6 py-4 bg-slate-50/50 flex flex-wrap items-center gap-2">
                    ...
                    <div className="ml-auto border border-slate-200 rounded-lg px-3 py-1.5 text-xs font-bold text-slate-500 bg-white flex items-center gap-2 shadow-2xs">
                      ...
                      <span>{quote?.quoteNumber || "QTE"}.pdf</span>
                    </div>
                  </div>
